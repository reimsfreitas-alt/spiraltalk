...
export type ConversationPolicy={
  scores:Record<PolicyAction,number>;
  primary:PolicyAction;
...
const empty:Record<PolicyAction,number>={
  listen:0,reflect:0,organize:0,answer:0,confront_gently:0,ask:0,pivot:0,close:0
};
...
export function decidePolicy(
  ...
  const scores={...empty};
  const recentQuestions=history.filter(m=>m.role==="assistant").slice(-4)
    .reduce((n,m)=>n+(m.content.match(/\?/g)||[]).length,0);
  const questionBudget=Math.max(0,2-recentQuestions);
  const corrections=userModel?.correctionRate||0;

  scores.listen+=2;
  scores.reflect+=signal.density>.35?3:1;

  // A concrete request is an answer obligation, not an invitation to ask more.
  ...
  if(questionBudget===0)scores.ask=-99;
  else if(signal.ambiguity>.25 && signal.intent!=="answer" && signal.intent!=="decide")scores.ask+=1;
  ...
  return {
    scores,primary,allowQuestion,questionBudget,
    answerFirst:signal.intent==="answer"||signal.intent==="decide",
    rationale:"ação="+primary+"; answerFirst="+(signal.intent==="answer"||signal.intent==="decide")+"; perguntas restantes="+questionBudget+"; correção_rate="+corrections.toFixed(2)
  };
}
